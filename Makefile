...
all : img/eda-ELO_update_with_result.png
...
data/elo_historic_wrangled.csv : data/elo_historic_raw.csv python src/wrangle.py
...
img/eda-correlation_map.png data/eda-elo_vs_result.csv img/eda-elo_lnrg.png img/eda-ELO_update_with_result.png : src/EDA_milestone3.Rmd data/elo_historic_raw.csv
	Rscript -e "rmarkdown::render('src/EDA_milestone3.Rmd', output_dir = 'doc/EDAindoc', params = list(source = '../data/elo_historic_raw.csv', correlation = '../img/eda-correlation_map.png', elo_vs_result = '../data/eda-elo_vs_result.csv', elo_lnrg = '../img/eda-elo_lnrg.png', elo_update = '../img/eda-ELO_update_with_result.png'))"

#Run random forest and logistic regression on the cleaned data
data/elo_2019_wrangled.csv : src/model.py data/elo_historic_wrangled.csv data/elo_2019_wrangled.csv
	python src/model.py --train_file=data/elo_historic_wrangled.csv --test_file=data/elo_2019_wrangled.csv

Elo_prediction_report.md : doc/Elo_prediction_report.Rmd
